...
run_client: ; unset https_proxy && unset http_proxy && docker run --rm --network host --env GNES_BENCHMARK_ID=$(GNES_BENCHMARK_ID) --env GNES_IMG_TAG=$(GNES_IMG_TAG) -v $(PWD)/.data:/workspace gnes/benchmark:client --mode index --num_bytes $(s) --num_docs $(d) --batch_size $(b)
